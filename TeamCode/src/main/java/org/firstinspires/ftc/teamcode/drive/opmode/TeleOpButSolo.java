...
import static org.firstinspires.ftc.teamcode.drive.opmode.Constants.horizontalSpeedLimit;
import static org.firstinspires.ftc.teamcode.drive.opmode.Constants.odometerDownPos;
import static org.firstinspires.ftc.teamcode.drive.opmode.Constants.odometerUpPos;
import static org.firstinspires.ftc.teamcode.drive.opmode.Constants.salamiSpeed;
...
public class TeleOpButSolo extends OpMode {
    ...
    boolean switchControlPrevious = false;

    public enum ControlMode{
        MOVE,
        FREIGHT;
        public ControlMode flip(){
            switch (this){
                case MOVE: return FREIGHT;
                case FREIGHT:
                default: return MOVE;
            }
        }
    ...
    public void switchControlMode(boolean keybind) {
        boolean switchControlCurrent = keybind;
        if (switchControlCurrent && !switchControlPrevious) {
            controlMode = controlMode.flip();
        }
        switchControlPrevious = switchControlCurrent;
    }

    public void liftOdometers(boolean req){
        if (req){
    ...
    public void drive(boolean slomo){
        ...
        float x2 = slomo ? (float) ((float) Math.pow(gamepad1.right_stick_x, 1) * salamiSpeed * 0.7) : (float) Math.pow(gamepad1.right_stick_x, 3);
